...
interface SourceDetailsSheetProps {
    ...
    selectedSource: KnowledgeSource | null | undefined;
}
...
const SourceDetailsSheet = ({isOpen, setIsOpen, selectedSource}:SourceDetailsSheetProps) => {
  ...
  return <Sheet open={isOpen} onOpenChange={setIsOpen}>
        <SheetContent className='w-full sm:max-w-md border-l border-white/10 bg-[#0A0A0E] p-0 shadiw-2xl'>
        <div className='h-full flex flex-col'>
            <SheetHeader className='p-6 border-b border-white/5'>
               <SheetTitle className='text-xl text-white flex items-center'>
                {
            ...
            <Button
             variant="destructive"
             className='w-full bg-red-500/10 text-red-500 hover:bg-red-300'
            >
                Disconnect Source
