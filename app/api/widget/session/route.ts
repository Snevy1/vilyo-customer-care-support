...
export async  function POST(req:Request){
    ...
    try {
        ...
        if(!widget_id){
            return NextResponse.json({error:"Missing widget_id"},{status:401})
        }
        ...
        const [bot] = await db.select().from(chatBotMetadata).where(eq(chatBotMetadata.id, widget_id)).limit(1)
         ...
         const token = await new SignJWT({widgetId: bot.id, ownerEmail:bot.user_email, sessionId}).setProtectedHeader({
            alg: "HS256"
