...
const data = await Promise.all((convs.map(async(c)=>{
    const [lastMessage] = await db.select().from(messages).where(eq(messages.conversation_id, c.id)).orderBy(desc(messages.created_at)).limit(1);
    ...
    if(ts){
        ...
        return {
            ...
            // email: c.user_email we may need this later
